...
//     const profileId = parseIntNullable(searchParams.get('profile_id')) || null;
...
export async function apiReady(req: Request, env: Env) {
    return sendResponse('Ready.');
}

export async function apiError(req: Request, env: Env): Promise<Response> {
...
export async function apiMatch(req: Request, env: Env) {
    const prisma = getPrisma();
    const { searchParams } = new URL(req.url);
...
export async function apiMatches(req: Request, env: Env) {
    ...
    const since = parseIntNullable(searchParams.get('since'));
    ...
    return sendResponse(matches.map(convMatch));
}
...
export async function apiLeaderboard(req: Request, env: Env) {
    ...
    const count = parseInt(searchParams.get('count') ?? '10');
    const leaderboardId = parseInt(searchParams.get('leaderboard_id')!);
...
export async function apiPlayerRatinghistory(req: Request, env: Env) {
    const prisma = getPrisma();
    const { searchParams } = new URL(req.url);
    ...
    return sendResponse(historyEntries.map(conv));
}

export async function apiProfile(req: Request, env: Env) {
    const prisma = getPrisma();
    const { searchParams } = new URL(req.url);
    ...
    const count = parseInt(searchParams.get('count') ?? '10');
    const steamId = searchParams.get('steam_id') || null;
    const profileId = parseIntNullable(searchParams.get('profile_id'));
    let search = searchParams.get('search') || null;
...
export async function apiPlayerMatches(req: Request, env: Env) {
    const prisma = getPrisma();
    const { searchParams } = new URL(req.url);

    const start = parseInt(searchParams.get('start') ?? '0') + 1; // old implementation detail
    const count = parseInt(searchParams.get('count') ?? '10');
    ...
    return sendResponse(matches2.map(convMatch2));
}
...
export async function apiNightbotRank(req: Request, env: Env) {
    const prisma = getPrisma();
    const { searchParams } = new URL(req.url);

    const leaderboardId = parseInt(searchParams.get('leaderboard_id') ?? '3');
    const language = 'en'; //searchParams.get('language') ?? 'en';
    const flag = (searchParams.get('flag') ?? 'true') === 'true';
    const search = searchParams.get('search') || null;
    const steamId = searchParams.get('steam_id') || null;
    const profileId = parseIntNullable(searchParams.get('profile_id')) || null;

    if (search == null && steamId == null && profileId == null) {
        return sendResponse('Missing search, steam_id, or profile_id');
    }

    const leaderboardRow = await getPlayer(leaderboardId, search, steamId, profileId);

    if (leaderboardRow == null) {
        return sendResponse('Player not found');
    }
    ...
    return sendResponseText(`${_flag} ${name} (${rating}) Rank #${rank}, has played ${games} games with a ${winrate}% winrate, ${streak} streak, and ${drops} drops`);
}
...
export async function apiNightbotMatch(req: Request, env: Env) {
    const prisma = getPrisma();
    const { searchParams } = new URL(req.url);
    ...
    const leaderboardId = parseInt(searchParams.get('leaderboard_id') ?? '3');
    const language = 'en'; //searchParams.get('language') ?? 'en';
    const color = (searchParams.get('color') ?? 'true') === 'true';
    const flag = false; //(searchParams.get('flag') ?? 'true') === 'true';
    const search = searchParams.get('search') || null;
    const steamId = searchParams.get('steam_id') || null;
    const profileId = parseIntNullable(searchParams.get('profile_id')) || null;
    ...
    if (leaderboardRow == null) {
        return sendResponse('Player for match not found');
    }
    ...
    if (match == null) {
        return sendResponseText('Match not found');
    }
    ...
    return sendResponseText(`${players} playing on ${map}`);
}
